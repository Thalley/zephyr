...
static void usb_data_request(const struct device *dev)
{
	...
	if (size != 0U) {
		...
		cnt++;
		LOG_DBG_RATELIMIT_RATE(USB_LOG_RATE, "[%zu]: Sending empty USB audio", cnt);
	}
...
static void bap_usb_send_frames_to_usb(void)
{
	...
	cnt++;
	LOG_DBG_RATELIMIT_RATE(USB_LOG_RATE, "[%zu]: Sending %zu USB audio frame", cnt, frame_cnt);
			     ...
			     size_t frame_size, uint32_t ts)
{
	...
	cnt++;
	LOG_DBG_RATELIMIT_RATE(USB_LOG_RATE, "[%zu]: Adding USB audio frame", cnt);
...
/* usb_out_data_mutex guards usb_out_left_ring_buffer, usb_out_right_ring_buffer, write_index
 * shell_stream.tx.left_read_idx and shell_stream.tx.right_read_idx
 */
...
static void stream_cb(struct shell_stream *sh_stream, void *user_data)
{
	if (sh_stream->is_tx) {
		...
		if (err != 0) {
			LOG_WRN("Failed to lock usb_out_data_mutex for new write index: %d", err);
			return;
		}
			     ...
			     void *user_data)
{
	...
	if (!out_terminal_enabled || size == 0U) {
		k_mem_slab_free(&usb_out_buf_pool, buf);
		return;
	}

	if (buf == NULL) {
		LOG_ERR("Unexpected NULL buf");
		return;
	}

	if (size != USB_STEREO_FRAME_SIZE) {
		LOG_WRN("Unexpected USB frame size: %zu (expected %u); dropping", size,
			USB_STEREO_FRAME_SIZE);
		k_mem_slab_free(&usb_out_buf_pool, buf);
		return;
	}
	...
	/* TODO: Move this to when it's read by the stream, not when new data comes in */
...
bool bap_usb_can_get_full_sdu(struct shell_stream *sh_stream)
{
	...
	err = k_mutex_lock(&usb_out_data_mutex, USB_OUT_DATA_MUTEX_TIMEOUT);
	if (err != 0) {
		LOG_WRN("Failed to lock usb_out_data_mutex to validate SDU availability: %d", err);
		return false;
	}
	...
	/* If we failed last time, we increase the requirement as a method to help stabilize it in
	 * case of underrun
	 */
	if (buffer_cnt <
	    (retrieve_cnt * (sh_stream->tx.failed_to_get_full_sdu_last_time ? 2U : 1U))) {
		/* Not enough for a frame yet */
		if (!sh_stream->tx.failed_to_get_full_sdu_last_time) {
		...
		sh_stream->tx.failed_to_get_full_sdu_last_time = true;

		return false;
	}

	sh_stream->tx.failed_to_get_full_sdu_last_time = false;
		       ...
		       int16_t buffer[])
{
	...
	if (err != 0) {
		...
		(void)memset(buffer, 0, frame_size);
		return;
	}
	...
	if (!is_mono) {
		const size_t diff = MAX(sh_stream->tx.right_read_idx, sh_stream->tx.left_read_idx) -
				    MIN(sh_stream->tx.right_read_idx, sh_stream->tx.left_read_idx);
		const bool recently_wrapped =
			MIN(sh_stream->tx.right_read_idx, sh_stream->tx.left_read_idx) < read_cnt;

		if (diff > read_cnt && !recently_wrapped) {
			LOG_ERR_RATELIMIT("Unexpected sh_stream->tx.right_read_idx %zu for "
					  "sh_stream->tx.left_read_idx %zu",
					  sh_stream->tx.right_read_idx,
					  sh_stream->tx.left_read_idx);
		}
	}
