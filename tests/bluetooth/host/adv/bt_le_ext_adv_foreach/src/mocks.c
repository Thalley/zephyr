...
struct net_buf *bt_hci_cmd_alloc(k_timeout_t timeout)
{
	...
	net_buf_simple_init_with_data(&test_cmd_buf.b, cmd_buf_storage, sizeof(cmd_buf_storage));
...
int bt_hci_cmd_send_sync(uint16_t opcode, struct net_buf *buf, struct net_buf **rsp)
{
	...
	if (rsp != NULL) {
		...
		net_buf_simple_add_mem(&test_rsp_buf.b, &cmd_rsp_payload, sizeof(cmd_rsp_payload));
...
/*
 * The real implementation defers the state update until the command completes.
 * As bt_hci_cmd_send_sync() above always succeeds, the state is applied right
 * away instead.
 */
