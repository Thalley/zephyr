...
The ISO Broadcast Benchmark sample tests the quality of a ISO broadcaster
against one or more ISO broadcast receivers, in terms for packet and sync loss.
The quality if measured by a receiver running this sample application, but in
the receiver role.
...
Sample output
==============
The receiver will output statistics about overall (since boot), current sync
