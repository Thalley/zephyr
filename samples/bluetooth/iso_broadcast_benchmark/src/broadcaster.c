...
{
	LOG_INF("ISO Channel %p connected (%u/%u)", chan, ++connected_bis,
...
static void iso_disconnected(struct bt_iso_chan *chan, uint8_t reason)
{
	...
	LOG_INF("ISO Channel %p disconnected with reason 0x%02x (%u/%u)",
	       chan, reason, big_create_param.num_bis - connected_bis,
	       big_create_param.num_bis);
